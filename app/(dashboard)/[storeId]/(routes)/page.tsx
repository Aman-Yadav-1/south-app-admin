...
import { collection, doc, getDocs, query, orderBy, limit, where, Timestamp } from "firebase/firestore";
...
const DashboardOverview = async ({ params, searchParams }: DashboardOverviewProps) => {
  try {
    ...
    const [
      ...
      getOrderPaymentStatusTotalRevenue(params.storeId, startTimestamp, endTimestamp),
      getOrderTotalRevenueByCategory(params.storeId, startTimestamp, endTimestamp)
    ]);
    ...
    const recentOrdersData = (
      await getDocs(recentOrdersQuery)
    ).docs.map((doc) => {
      const data = doc.data();
      return {
        ...data,
    ...
    const inventoryStats = {
      ...
      lowStock: inventoryItems.filter(item => (item as any).quantity <= ((item as any).minQuantity || 5)).length,
      totalValue: inventoryItems.reduce((sum, item) => sum + (((item as any).quantity || 0) * ((item as any).cost || 0)), 0)
    };
      ...
      <div className="flex-col">
        <div className="flex-1 space-y-6 p-6 pt-4">
          ...
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 text-red-800 dark:text-red-200">
            ...
            <p className="mt-1">There was a problem loading your dashboard data. Please try again later.</p>
          </div>
        </div>
      </div>
    );
